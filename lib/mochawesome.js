...
var mocha = require('mocha'),
    ...
    codeceptjsConf = require('../../../codecept.conf').config
        ...
        try {
            if (!endCalled) {
                ...
                var obj = {
                    ...
                    browser: codeceptjsConf.helpers.Appium.browser,
                    windowSize: codeceptjsConf.helpers.Appium.windowSize,
                    baseUrl: codeceptjsConf.helpers.Appium.url
                };

                obj.stats.tests = 0;
                obj.stats.passes = 0;
                obj.stats.pending = 0;
                obj.stats.failures = 0;
                obj.stats.skipped = 0;


                for (var i = 0; i < allSuites.suites.length; i++) {
                    if (allSuites.suites[i].totalTests === (allSuites.suites[i].totalSkipped + allSuites.suites[i].totalPending)) {
                        delete obj.suites.suites[i]
                        obj.suites.suites.splice(i, 1);
                        i--;
                    } else {
                        obj.stats.tests = obj.stats.tests + allSuites.suites[i].totalTests - allSuites.suites[i].totalSkipped;
                        obj.stats.passes = obj.stats.passes + allSuites.suites[i].totalPasses;
                        obj.stats.pending = obj.stats.pending + allSuites.suites[i].totalPending;
                        obj.stats.failures = obj.stats.failures + allSuites.suites[i].totalFailures;
                        obj.stats.skipped = obj.stats.skipped + allSuites.suites[i].totalSkipped;
                    }
                }
                ...
                obj.stats.testsRegistered = obj.stats.tests
...
function cleanTest(test) {
    ...
    if ("err" in test) {
        if ("template" in test.err) {
            var detailed = test.err.template
                //      {{customMessage}}expected {{jar}} {{type}} {{needle}}
                .replace('{{customMessage}}', test.err.params.customMessage)
                .replace('{{type}}', test.err.params.type)
                .replace('{{jar}}', test.err.params.jar)
                .replace('{{needle}}', test.err.params.needle)
                .replace('{{expected}}', test.err.params.expected)
                .replace('{{actual}}', test.err.params.actual)
                .replace('{{subject}}', test.err.params.subject)
                .replace('{{value}}', test.err.params.value)

            if ((test.err.params.type.indexOf("to be seen") > -1) || (test.err.params.type.indexOf("to include") > -1)) {
                err.message = detailed
            } else {
                err.message = detailed + err.message
                err.stack = detailed + '\n\nactual:\n' + test.err.actual + '\n\nexpected:\n' + test.err.expected + err.stack
            }
        }
        var coloredString = err.stack
        var refactor = coloredString.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, "")
        err.stack = refactor
        if (err.message.indexOf("misMatchPercentage") > -1) {
            err.screenshot = err.message.split('Save screenshot to: ')[1];
        } else {
            err.screenshot = test.title.substring(0, 10).replace(/ /g, '_') + '-' + hashCode(test.title) + '-' + hashCode(test.file) + '.failed.png'
        }
    }
